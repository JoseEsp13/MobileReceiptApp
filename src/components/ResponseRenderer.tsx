import React from 'react';
import {StyleSheet, View, Text} from 'react-native';
import {Block, Line, ITextRecognitionResponse} from './mlkit';

interface ResponseRendererProps {
  response?: ITextRecognitionResponse;
  scale: number;
  windowWidth: number,
  aspectRatio: number,
}

export type Size = {
  width: number;
  height: number;
};

export const ResponseRenderer = (props: ResponseRendererProps) => {
  return (
    <>
      {/*<View
        style={{
          backgroundColor: 'transparent',
          width: props.windowWidth,
          height: props.windowWidth * props.aspectRatio
        }}>
        {props.response?.blocks.map((block) => {
          return block.lines.map((line, index) => {
            return (
              <>
                <Text key={index}>{line.text}</Text>
                {<BlockComponent block={line} scale={props.scale} key={`key${index}`} />}
              </>
            );
          });
        })}
      </View> */}
      {props.response?.blocks.map((block) => {
        return block.lines.map((line, index) => {
          return (
            <Text key={index} style={{color: "black"}}>{line.text}</Text>
          );
        });
      })}
    </>
  );
};

type BlockProps = {
  block: Block | Line;
  scale: number;
};

export const BlockComponent = ({block, scale}: BlockProps) => {
  const rect = {
    top: block.rect.top * scale,
    width: block.rect.width * scale,
    left: block.rect.left * scale,
    height: block.rect.height * scale,
  };

  return (
    <View
      style={{
        position: 'absolute',
        ...rect,
        borderWidth: 1,
        borderColor: 'red',
      }}>
      <Text style={{color: 'blue'}}>{block.text}</Text>
    </View>
  );
};
