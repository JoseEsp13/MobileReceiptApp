...
function parseCostco(response: ITextRecognitionResponse): { [key: string]: number } {
  ...
  for (var i = items.length - 1; i >= 0; i--) {
    let current = items[i]
    if (total_hit && item_count < num_items && !parseTools.isSubtotal(current) && !parseTools.isStringDiscount(current) && !parseTools.isPrice(current) && !parseTools.isDiscount(current) && !parseTools.isChar(current)) {
